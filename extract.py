...
def load_areas(cursor: connection):
    ...
    for area_nome in areas:
        ...
        cursor.execute(f'INSERT INTO "AREA" VALUES ({area_nome}) RETURNING id')
        area_id = cursor.fetchone()[0]

        if tipo == 'Country':
            cursor.execute(f'INSERT INTO "PAIS" (id, codigo, nome) VALUES ({area_id},{code},{area_nome})')
        elif tipo == 'Group':
            cursor.execute(f'INSERT INTO "GRUPO" (id, codigo, nome) VALUES ({area_id},{code},{area_nome})')
